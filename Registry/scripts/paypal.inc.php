	...
	function GetButtonCode() {
		if (strlen($this->accountemail) > 0 && (count($this->items) > 0 || count($this->subscriptions) > 0)) {
			...
			if (count($this->items) > 0) {
				...
				if (count($this->items) == 1) {
					...
					if (strlen($this->items[0]['item_no']) > 0) {
						$out = $out. '<input type="hidden" name="item_number" value="' . $this->items[0]['item_no'] . '" />';
					}
					//shipping
					if (strlen($this->items[0]['shipping']) > 0) {
						$out = $out. '<input type="hidden" name="shipping" value="' . $this->items[0]['shipping'] . '" />';
					}
					//shipping2
					if (strlen($this->items[0]['shipping2']) > 0) {
						$out = $out. '<input type="hidden" name="shipping2" value="' . $this->items[0]['shipping2'] . '" />';
					}
					//handling
					if (strlen($this->items[0]['handling']) > 0) {
						$out = $out. '<input type="hidden" name="handling" value="' . $this->items[0]['handling'] . '" />';
					}
					//first field name
					if (strlen($this->items[0]['firstfieldname']) > 0) {
						$out = $out. '<input type="hidden" name="on0" value="' . $this->items[0]['firstfieldname'] . '" />';
					}
					//first field options
					if (strlen($this->items[0]['firstfieldoptions']) > 0) {
						$out = $out. '<input type="hidden" name="os0" value="' . $this->items[0]['firstfieldoptions'] . '" />';
					}
					//second field name
					if (strlen($this->items[0]['secondfieldname']) > 0) {
						$out = $out. '<input type="hidden" name="on1" value="' . $this->items[0]['secondfieldname'] . '" />';
					}
					//second field options
					if (strlen($this->items[0]['secondfieldoptions']) > 0) {
						$out = $out. '<input type="hidden" name="os1" value="' . $this->items[0]['secondfieldoptions'] . '" />';
					}
					...
					//more than 1 item
					$out = $out. '<input type="hidden" name="cmd" value="_cart" />';
					$out = $out. '<input type="hidden" name="upload" value="1" />';
					$out = $out. '<input type="hidden" name="item_name" value="Shopping Cart" />';
					$totalamount = 0;
					for ($x=0;$x<count($this->items);$x++) {
						//for each item
						$out = $out. '<input type="hidden" name="item_name_' . ($x+1) . '" value="' . $this->items[$x]['name'] . '" />';
						$out = $out. '<input type="hidden" name="quantity_' . ($x+1) . '" value="' . $this->items[$x]['qty'] . '" />';
						$out = $out. '<input type="hidden" name="amount_' . ($x+1) . '" value="' . $this->items[$x]['price'] . '" />';
						//item no
						if (strlen($this->items[$x]['item_no']) > 0) {
							$out = $out. '<input type="hidden" name="item_number_' . ($x+1) . '" value="' . $this->items[$x]['item_no'] . '" />';
						}
						//shipping
						if (strlen($this->items[$x]['shipping']) > 0) {
							$out = $out. '<input type="hidden" name="shipping_' . ($x+1) . '" value="' . $this->items[$x]['shipping'] . '" />';
						}
						//shipping2
						if (strlen($this->items[$x]['shipping2']) > 0) {
							$out = $out. '<input type="hidden" name="shipping2_' . ($x+1) . '" value="' . $this->items[$x]['shipping2'] . '" />';
						}
						//handling
						if (strlen($this->items[$x]['handling']) > 0) {
							$out = $out. '<input type="hidden" name="handling_' . ($x+1) . '" value="' . $this->items[$x]['handling'] . '" />';
						}
						//tax
						if (strlen($this->items[$x]['tax']) > 0) {
							$out = $out. '<input type="hidden" name="tax_' . ($x+1) . '" value="' . $this->items[$x]['tax'] . '" />';
						}
						//first field name
						if (strlen($this->items[$x]['firstfieldname']) > 0) {
							$out = $out. '<input type="hidden" name="on0_' . ($x+1) . '" value="' . $this->items[$x]['firstfieldname'] . '" />';
						}
						//first field options
						if (strlen($this->items[$x]['firstfieldoptions']) > 0) {
							$out = $out. '<input type="hidden" name="os0_' . ($x+1) . '" value="' . $this->items[$x]['firstfieldoptions'] . '" />';
						}
						//second field name
						if (strlen($this->items[$x]['secondfieldname']) > 0) {
							$out = $out. '<input type="hidden" name="on1_' . ($x+1) . '" value="' . $this->items[$x]['secondfieldname'] . '" />';
						}
						//second field options
						if (strlen($this->items[$x]['secondfieldoptions']) > 0) {
							$out = $out. '<input type="hidden" name="os1_' . ($x+1) . '" value="' . $this->items[$x]['secondfieldoptions'] . '" />';
						}
					...
					$out = $out. '<input type="hidden" name="amount" value="' . number_format($totalamount,2,".","") . '" />';
				}
				//END ITEMS
			...
			//END PURCHASE BUTTON
				...
				$out = $out. '<input type="hidden" name="cmd" value="_xclick-subscriptions" />';
				$out = $out. '<input type="hidden" name="modify" value="1" />'; //??
				$out = $out. '<input type="hidden" name="bn" value="PP-SubscriptionsBF" />'; //???
				$out = $out. '<input type="hidden" name="src" value="1" />'; //???
				$out = $out. '<input type="hidden" name="sra" value="1" />'; //???
				
				$out = $out. '<input type="hidden" name="item_name" value="' . $this->subscriptions[0]['name'] . '" />';
				$out = $out. '<input type="hidden" name="a3" value="' . $this->subscriptions[0]['price'] . '" />';
				//item no
				if (strlen($this->subscriptions[0]['item_no']) > 0) {
					$out = $out. '<input type="hidden" name="item_number" value="' . $this->subscriptions[0]['item_no'] . '" />';
				}
				//time_period
				if (strlen($this->subscriptions[0]['time_period']) > 0) {
					$out = $out. '<input type="hidden" name="t3" value="' . $this->subscriptions[0]['time_period'] . '" />';
				}
				//interval
				if (strlen($this->subscriptions[0]['interval']) > 0) {
					$out = $out. '<input type="hidden" name="p3" value="' . $this->subscriptions[0]['interval'] . '" />';
				}
			...
			$out = $out. '<input type="hidden" name="business" value="' . $this->accountemail . '" />';
			$out = $out. '<input type="hidden" name="currency_code" value="' . $this->currencycode . '" />';

			//custom
			if (strlen($this->custom) > 0) {
				$out = $out. '<input type="hidden" name="custom" value="' . $this->custom . '" />';
			}
			
			//image
			if (strlen($this->image) > 0) {
				$out = $out. '<input type="hidden" name="image_url" value="' . $this->image . '" />';
			}		
		
			//askforaddress
			if ($this->askforaddress == true) {
				//ask for address
				$out = $out. '<input type="hidden" name="no_shipping" value="0" />';
			} else {
				//don't ask for address
				$out = $out. '<input type="hidden" name="no_shipping" value="1" />';
			}
			
			//note
			if ($this->askfornote == true) {
				//ask for note
				$out = $out. '<input type="hidden" name="no_note" value="0" />';
			} else {
				//don't ask for note
				$out = $out. '<input type="hidden" name="no_note" value="1" />';
			}
			
			//reutn url
			if (strlen($this->return_url) > 0) {
				$out = $out. '<input type="hidden" name="return" value="' . $this->return_url . '" />';
			}
			//ipn url
			if (strlen($this->ipn_url) > 0) {
				$out = $out. '<input type="hidden" name="notify_url" value="' . $this->ipn_url . '" />';
			}
			//cancel url
			if (strlen($this->cancel_url) > 0) {
				$out = $out. '<input type="hidden" name="cancel_return" value="' . $this->cancel_url . '" />';
			}
			//class css
			...
			if (strlen($this->buttonimage) > 0) {
				$out = $out. '<input type="image" src="' . $this->buttonimage . '" name="submit" alt="' . $this->buttontext . '"' . $class . $width . ' />';
			} else {
				$out = $out. '<input type="submit" value="' . $this->buttontext . '" name="submit"' . $class . $width . ' />';
			}
			$out = $out. '</form>';
		} else {
			//not ok
			$out = $out. 'Error: $accountemail directive not set, or no items to sell!';
		}
    return $out;
	}
